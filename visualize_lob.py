...
def _make_ob_row(code, adj_index, time, server_time, bids, asks, depth=10):
    ...
    参数:
        ...
        adj_index: 该帧的索引(int).
        time: 交易所时间(datetime).
        server_time: 本地收到行情的时间(datetime).
...
def generate_toy_data(out_dir="toy_data", n_events=100, code=600519, seed=42):
    ...
    ts = pd.Timestamp("2026-05-15 09:30:00")        # 起始交易所时间(集合竞价后开盘)
    adj = int(rng.integers(1, 5))                   # 起始 adjIndex(随机起点)

    # ---- 记录初始帧(第 0 帧, 无触发事件) ----
    bids, asks = ob.snapshot(10)                    # 取初始快照
    server_ts = ts + pd.Timedelta(milliseconds=int(rng.integers(1, 8)))  # 本地时间略晚于交易所时间
    ob_rows.append(_make_ob_row(code, adj, ts, server_ts, bids, asks))   # 写入第 0 帧
    ...
    while len(tr_rows) < n_events and guard < 1_000_000:
        ...
        ts = ts + pd.Timedelta(milliseconds=int(rng.integers(50, 800)))  # 交易所时间前进
        adj += int(rng.integers(1, 5))              # adjIndex 单调递增, 步长 1~4(允许跳号)
        server_ts = ts + pd.Timedelta(milliseconds=int(rng.integers(1, 8)))  # 本地时间略晚
        ob_rows.append(_make_ob_row(code, adj, ts, server_ts, bids, asks))   # 写入 orderbook 行
...
class LOBDataLoader:
    ...
    def filter(self, code=None, start_time=None, end_time=None,
        ...
        if code is not None:                        # 指定了股票代码
            df = df[df["code"] == code]
        if start_time is not None:                  # 指定了起始时间
            df = df[df["time"] >= pd.Timestamp(start_time)]
        if end_time is not None:                    # 指定了结束时间
...
class LOBVisualizer:
    ...
    def plot_single_frame(self, pos):
        ...
        title = (f"订单簿快照　第 {pos} 帧　|　code={int(row['code'])}　"
                 f"adjIndex={int(row['adjIndex'])}　|　时间 {row['time']}　|　"
                 f"触发类型: {ttype}")
