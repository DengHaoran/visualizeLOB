...
from typing import Optional, Tuple, Dict, List, Union             # 类型注解
...
def generate_toy_data(
    ...
    code: int = 1,                                                 # 股票代码（整型，展示时可格式化为 6 位）
    ...
    # ---- 逐事件模拟 ----
    for i in range(1, n_events * 10 + 1):                          # 多尝试一些以确保有足够帧
        if recorded_events >= n_events:                            # 已达到目标事件帧数
            break                                                  # 停止生成
        ...
        # ======== 记录新帧 ========
        adj_index += int(np.random.randint(1, 5))                  # 帧索引单调递增，但允许跳号
...
class LOBDataLoader:
    ...
    def filter(
        self,
        code: Optional[Union[int, str]] = None,                    # 股票代码（支持 int / str 输入）
        ...
        if code is not None:                                       # 按股票代码
            normalized_code = int(code)                            # 统一转为整型，兼容 "000001" / 1
            ob_codes = pd.to_numeric(df["code"], errors="coerce")  # 兼容历史字符串 code 数据
            trigger_codes = pd.to_numeric(tdf["code"], errors="coerce")  # 同上
            df = df[ob_codes == normalized_code]                   # 筛选订单簿
            tdf = tdf[trigger_codes == normalized_code]            # 筛选触发信息
...
class LOBVisualizer:
    ...
    def _format_code(self, code_value: Union[int, str]) -> str:
        """
        将股票代码格式化为 6 位展示字符串

        输入数据层面使用 int，展示层面补零为 6 位，更贴近行情终端习惯。
        """
        return f"{int(code_value):06d}"                           # 例如 1 → 000001
    ...
    def _build_stacked_frame_traces(
        self,
        pos: int,                                                  # 帧位置（0-based）
        all_prices: List[float],                                   # 全局统一价格坐标
        show_legend: bool = True,                                  # 是否显示图例
        highlight_changes: bool = True,                            # 是否高亮变化段
        stage_label: str = "当前帧",                               # 阶段标签（当前帧/变化阶段/平移阶段）
        ...
        BAR_WIDTH = 0.008                                          # 柱宽（价格轴 0.01 tick 略窄）
        ...
        for px in all_prices:                                      # 遍历全局价格坐标
            label = f"{px:.2f}"                                   # 用于悬停展示的价格文本
            ...
            if cb > 0 or pb > 0:                                   # 该价位存在买盘数据
                if highlight_changes and cb > pb:                  # 量增加（新挂单 / partial fill）
                ...
                elif highlight_changes and cb < pb:                # 量减少（撤单 / 被成交）
            ...
            if ca > 0 or pa > 0:                                   # 该价位存在卖盘数据
                if highlight_changes and ca > pa:                  # 量增加
                ...
                elif highlight_changes and ca < pa:                # 量减少
        ...
        title = (                                                  # 主标题
            f"订单簿 [{self._format_code(current['code'])}]　|　"
            f"帧 #{adj_idx}　|　"
            f"时间 {time_str}"
        )
        ...
        stage_annotation = dict(                                   # 阶段说明注释
            text=f"<b>阶段: {stage_label}</b>",                    # 当前动画阶段
        ...
        annotations = [trigger_annotation, stage_annotation, legend_annotation]  # 注释列表
    ...
    def _get_all_price_labels(
        ...
        return sorted(all_prices)                                  # 排序返回浮点价格

    def _get_mid_price(self, frame: pd.Series) -> float:
        """
        计算一帧订单簿的中间价，用于动画的整体平移视窗中心。
        """
        best_bid = frame.get("bidPx1")                             # 买一价
        best_ask = frame.get("askPx1")                             # 卖一价
        if pd.notna(best_bid) and pd.notna(best_ask):              # 有完整盘口
            return (float(best_bid) + float(best_ask)) / 2.0       # 中间价

        visible_prices: List[float] = []                           # 兜底价格列表
        for side in ["bid", "ask"]:                             # 遍历两侧
            for lv in range(1, MAX_DEPTH + 1):                     # 10 档
                px = frame.get(f"{side}Px{lv}")                    # 价格
                vlm = frame.get(f"{side}Vlm{lv}", 0)              # 数量
                if pd.notna(px) and vlm > 0:                       # 有效价量
                    visible_prices.append(float(px))               # 收集价格

        if visible_prices:                                         # 有价格时
            return float(np.mean(visible_prices))                  # 用均值兜底
    ...
    def plot_single_frame(self, pos: int) -> go.Figure:
        ...
        traces, title, anns = self._build_stacked_frame_traces(    # 构建堆叠 traces
            pos, price_labels, show_legend=True, highlight_changes=True, stage_label="当前帧"
        )
    ...
    def plot_animation(
        ...
        # ---- 全局价格坐标（用于柱体定位） ----
        all_prices = self._get_all_price_labels(start_pos, end_pos)

        # ---- 计算固定视窗半宽（平移时只移动中心，不改变缩放比例） ----
        half_width = 0.06                                          # 最小半宽，保证可视范围足够
        for p in range(start_pos, end_pos + 1):                    # 遍历帧
            frame = self.loader.get_frame(p)                       # 当前帧
            mid_price = self._get_mid_price(frame)                 # 当前中间价
            for side in ["bid", "ask"]:                         # 买卖两侧
                for lv in range(1, MAX_DEPTH + 1):                 # 10 档
                    px = frame.get(f"{side}Px{lv}")                # 价格
                    vlm = frame.get(f"{side}Vlm{lv}", 0)          # 数量
                    if pd.notna(px) and vlm > 0:                   # 有效档位
                        dist = abs(float(px) - mid_price) + 0.02   # 给一点边距
                        if dist > half_width:                      # 更新半宽
                            half_width = dist
        ...
        # ---- 构建初始帧 ----
        init_frame = self.loader.get_frame(start_pos)              # 初始帧
        init_center = self._get_mid_price(init_frame)              # 初始视窗中心
        ...
        for p in range(start_pos, end_pos + 1):                    # 遍历每个订单簿帧
            current_frame = self.loader.get_frame(p)               # 当前帧
            current_center = self._get_mid_price(current_frame)    # 当前视窗中心

            if p > start_pos:                                      # 非首帧才有变化阶段
                prev_frame = self.loader.get_frame(p - 1)          # 上一帧
                prev_center = self._get_mid_price(prev_frame)      # 上一帧视窗中心
                delta_traces, delta_title, delta_anns = (          # 变化阶段 traces
                    self._build_stacked_frame_traces(
                        p,
                        all_prices,
                        show_legend=False,
                ...
                delta_name = f"{p}-delta"                         # 变化阶段帧名
                frames.append(go.Frame(                            # 变化阶段帧
                    data=delta_traces,
                    name=delta_name,
                    layout=dict(
                        title=dict(text=delta_title),
                        annotations=delta_anns,
                        xaxis=dict(range=[prev_center - half_width, prev_center + half_width]),
                    ),
                ))

            settle_traces, settle_title, settle_anns = (           # 平移阶段 traces
                self._build_stacked_frame_traces(
                    p,
                    all_prices,
                    show_legend=False,
                    highlight_changes=True,
                    stage_label="平移阶段（保留变化标记）",
                )
            )
            settle_name = f"{p}-settle"                           # 平移阶段帧名
            frames.append(go.Frame(                                # 平移阶段帧
                data=settle_traces,
                name=settle_name,
                layout=dict(
                    title=dict(text=settle_title),
                    annotations=settle_anns,
                    xaxis=dict(range=[current_center - half_width, current_center + half_width]),
                ),
            ))

            adj_idx = int(current_frame["adjIndex"])              # 显示索引
            slider_steps.append(dict(                              # 滑块仅跳转到平移后的稳定帧
                args=[[settle_name], dict(
        ...
        fig = go.Figure(                                           # 创建 Figure
            ...
            layout=go.Layout(                                      # 布局
                ...
                xaxis=dict(                                        # x 轴
                    ...
                    range=[init_center - half_width, init_center + half_width],
                ),
                yaxis=dict(                                        # y 轴
                    title="数量",
